...
function &main_form($sel_year, $sel_seme, $class_id, $stud_id) {
	...
	if (! empty ( $class_id )) {
		...
		$class_num = $class [2];
		$grid1->sql_str = "select a.stud_id,a.stud_name,a.stud_sex,b.seme_num as sit_num from stud_base a,stud_seme b where a.stud_id=b.stud_id  and a.stud_study_cond=0 and  b.seme_year_seme='$seme_class_seme' and b.seme_class='$class_num' order by b.seme_num "; // SQL 命令
		...
		if (! empty ( $stud_id )) {
			...
			$stud_all = "<b>" . $stu [stud_name] . "（" . $stu_class_num [num] . "號）</b>的成績資料如下：<a href='" . $SFS_PATH_HTML . "modules/score_paper/input.php?stud_id=$stud_id&sel_year=$sel_year&sel_seme=$sel_seme&class_id=$class_id'>輸入" . $stu [stud_name] . "的成績單</a><br>
			 ...
			 * $sign_1['簽章'] ="<draw:image draw:style-name=\"fr1\" draw:name=\"signature1\" text:anchor-type=\"paragraph\" svg:width=\"1.27cm\" svg:height=\"1.27cm\" draw:z-index=\"0\" xlink:href=\"$title_img\" xlink:type=\"simple\" xlink:show=\"embed\" xlink:actuate=\"onLoad\"/>";
			 * } else {
			 * $sign_1['簽章']="找不到";
			 * }
			 ...
			 *
			 * print($sign_1).'<br>';
			 * $stud_all.=make_list($sign_1,"簽章情況","","",false);
			 */
			// 簽章情況 add by Brando 2017-01-09
			// $school_sign_stamp=$get_school_base_array();
			// $stud_all.="<td>".$school_sign_stamp."/<td>";
			// print($school_sign_stamp).'<br>';
			// $stud_all.=make_list($school_sign_stamp,"簽章情況","","",false);
...
function &get_score_value2($sel_year, $sel_seme, $stud_id, $student_sn, $class_id, $oth_data) {
	...
	foreach ( $ss9 as $link_ss ) {
		...
		$yss9 [$k] = $link_ss;
		$other9 [$k] = array (
				str_replace ( "{break_text}", "<br>", $ss9_array [$k1] . "節" ),
...
function make_ooo($sel_year, $sel_seme, $class_id, $sp_sn, $data_arr) {
	...
	foreach ( $data_arr as $student_sn => $stud_id ) {
		...
		$score_array ["生活主軸_學期總平均加權"] = $score_array ["生活主軸-生活技能加權"] + $score_array ["生活主軸-群己活動加權"] + $score_array ["生活主軸-理念學習加權"] + $score_array ["生活主軸-生活規範加權"] + $score_array ["生活主軸-健康生活加權"] + $score_array ["生活主軸-服務學習加權"];
		
		$score_array ["生活主軸_學期總分數"] = $score_array ["生活主軸-生活技能分數"] * $score_array ["生活主軸-生活技能加權"] + $score_array ["生活主軸-群己活動分數"] * $score_array ["生活主軸-群己活動加權"] + $score_array ["生活主軸-理念學習分數"] * $score_array ["生活主軸-理念學習加權"] + $score_array ["生活主軸-生活規範分數"] * $score_array ["生活主軸-生活規範加權"] + $score_array ["生活主軸-健康生活分數"] * $score_array ["生活主軸-健康生活加權"] + $score_array ["生活主軸-服務學習分數"] * $score_array ["生活主軸-服務學習加權"];
		...
		//2017-12-15 Update By Brando for 刪除德討
		$score_array ["生命主軸_學期總平均加權"] = $score_array ["生命主軸-生命課程加權"];
		...
		//2017-12-15 Update By Brando for 刪除德討
		$score_array ["生命主軸_學期總分數"] = $score_array ["生命主軸-生命課程分數"] * $score_array ["生命主軸-生命課程加權"];
		// 2017-01-08 Update by Brando for 德育實踐檢討
		...
		//2017-12-15 Update By Brando for 刪除基礎課程
		$score_array ["基礎主軸_學期總平均加權"] = $score_array ["基礎主軸-經典背誦加權"] + $score_array ["基礎主軸-書法硬筆字加權"];
		$score_array ["基礎主軸_學期總分數"] = $score_array ["基礎主軸-經典背誦分數"] * $score_array ["基礎主軸-經典背誦加權"] + $score_array ["基礎主軸-書法硬筆字分數"] * $score_array ["基礎主軸-書法硬筆字加權"];
		//$score_array ["基礎主軸_學期總平均加權"] = $score_array ["基礎主軸-基礎課程加權"] + $score_array ["基礎主軸-經典背誦加權"] + $score_array ["基礎主軸-書法硬筆字加權"];
		//$score_array ["基礎主軸_學期總分數"] = $score_array ["基礎主軸-基礎課程分數"] * $score_array ["基礎主軸-基礎課程加權"] + $score_array ["基礎主軸-經典背誦分數"] * $score_array ["基礎主軸-經典背誦加權"] + $score_array ["基礎主軸-書法硬筆字分數"] * $score_array ["基礎主軸-書法硬筆字加權"];
		$score_array ["基礎主軸_學期總平均分數"] = round ( $score_array ["基礎主軸_學期總分數"] / $score_array ["基礎主軸_學期總平均加權"], 2 );
		$score_array ["基礎主軸_學期總平均等第"] = score2str ( $score_array ["基礎主軸_學期總平均分數"], $class );

...
$Submit7=($_GET['Submit7'])?$_GET['Submit7']:$_POST['Submit7'];
...
//網頁內容請置於此處--------------------------------------------------------------
    ...
    //複製階段選單
    $stage_str="";
    if($year_seme && ($year_name)&&($me)&&($scope_subject)&&($stage_score)&&($stage_score!="all")){
        $stage_str.="<td><form name='form7' method='post' action='{$_SERVER['PHP_SELF']}?year_name=$year_name&me=$me&scope_subject=$scope_subject&stage_score=$stage_score'>";
        if( preg_match("3",$select_stage_score) ) {
        	if ($stage_score == '1'){
						$stage_str = "<input type='checkbox' name='stage_list[]' value='2'>第2階段<input type='checkbox' name='stage_list[]' value='3'>第3階段";        	
        	}
        	elseif ($stage_score == '1'){
						$stage_str = "<input type='checkbox' name='stage_list[]' value='1'>第1階段<input type='checkbox' name='stage_list[]' value='3'>第3階段";        		
        	}
        	else {
        		$stage_str = "<input type='checkbox' name='stage_list[]' value='1'>第1階段<input type='checkbox' name='stage_list[]' value='2'>第2階段";	
        	}          
        }  
        else{
        	if ($stage_score == '1'){
        	  $stage_str = "<input type='checkbox' name='stage_list[]' value='2'>第2階段";
          }
          else{
            $stage_str = "<input type='checkbox' name='stage_list[]' value='1' >第1階段";
          }
        }
        $stage_str.="  <input type='submit' name='Submit7' value='開始'>
                     <input type='hidden' name='year_seme' value='$year_seme'></form></td>";
    }	
        
    $menu="
        <table cellspacing=0 cellpadding=0>
            <tr>
                <td>$year_seme_menu</td><td>$class_year_menu</td><td>$class_year_name_menu</td><td>$scope_subject_menu</td><td>$stage_score_menu</td>$stage_str
            </tr>
        </table>";
    echo $menu;
    //echo $stage_score;
...
function jumpMenu5(){
	var str, classstr ;
  if ((document.form5.year_name.value!="") & (document.form5.me.value!="") & (document.form5.scope_subject.value!="") & (document.form5.stage_score.options[document.form5.stage_score.selectedIndex].value!="") & (document.form5.stage_list.length >0)) {
  	alert(document.form5.stage_list.length);
	}
	else{
		alert("error");
  }	

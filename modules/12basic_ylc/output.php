...
if($_POST['act']){
	...
	//製作抬頭
	//$x->filename=$SCHOOL_BASE['sch_id'].'_'.$school_long_name.'_學生資料檔.xls';
	switch($_POST['act']){
		case 'EXCEL':
			$x=new sfs_xls();
			$x->setUTF8();
	...
	while(!$recordSet->EOF){
		...
		$stud_country_kind=($student_data[$student_sn]['stud_country_kind']>0)?'V':'';		//身分證照種類
		...
		//計算12basic_ylc紀錄資料
		$kind_id=$final_data[$student_sn]['kind_id'];	//學生身分
		$apply_id=($final_data[$student_sn]['kind_id']>0)?'':'0';		//報名身分
...
$data="※要輸出的格式：<input type='submit' name='act' value='HTML' onclick=\"document.myform.target='$academic_year'\"> <input type='submit' name='act' value='EXCEL' onclick=\"document.myform.target=''\">";
